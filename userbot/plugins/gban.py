...
D15H4NT0P = str(ALIVE_NAME) if ALIVE_NAME else "vamp User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        vampBot = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        vampBot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await vampBot.edit(f"`🔥Global Ban Iz Cumin'💦.... Wait and watch nigga🚶`")
    ...
    try:
        ...
        return await vampBot.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 1869741341 or user.id == 1837224931 or user.id == 1642269125:
            return await vampBot.edit(
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await vampBot.edit(f"Gbaning This retard🚶\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
        await vampBot.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await vampBot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await vampBot.edit(
        f"[{user.first_name}](tg://user?id={user.id}) Beta majdur ko khodna aur [{D15H4NT0P}](tg://user?id={papa}) ko chodna kabhi sikhana nhi.\n\n**Gban Successful 🔥\nAffected Chats😏 : {a} **"
    )
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        vampBot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        vampBot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await vampBot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await vampBot.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 1869741341 or user.id == 1837224931 or user.id == 1642269125:
            return await vampBot.edit("**You need to grow some balls to gban / ungban my creator**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await vampBot.edit(f"Ok! Now Ungbaning this kiddo.\nChats:- `{a}`")
            except:
                b += 1
    else:
        await vampBot.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await vampBot.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await vampBot.edit(
...
@borg.on(ChatAction)
async def handler(vamp): 
   if vamp.user_joined or vamp.user_added:      
       try:       	
         from userbot.plugins.sql_helper.gmute_sql import is_gmuted
         guser = await vamp.get_user()      
