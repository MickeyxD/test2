...
async def _(event):
    ...
    vampevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_svamp(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    vampuser = await event.client.get_me()
    if vampuser.username:
        curruser = vampuser.username
    else:
        curruser = "vampBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    vampevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on vampBot server and shows details'
).add_command(
