...
async def _(event):
    ...
    process = await asyncio.create_subprocess_svamp(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    OUTPUT = f"**Files in [vampBot](t.me/VAMPBOT_OFFICIAL) DOWNLOADS Folder:**\n"
...
CmdHelp("filemanager").add_command(
  'ls_local', None, 'Gives the list of downloaded medias in your vampBot server.'
).add_command(
  'ls_root', None, 'Gives the list of all files in root directory of vampBot repo.'
).add_command(
  'ls_saved', None, 'Gives the list of all files in Saved directory of your vampBot server'
).add_command(
