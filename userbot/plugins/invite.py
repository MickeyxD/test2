...
async def get_users(event):
    ...
    if not sender.id == me.id:
        ...
        vamp = await edit_or_reply(event, "`processing...`")
    vamp = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await vamp.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await vamp.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(vamp.full_chat.id):
        try:
            if error.startswith("Too"):
                return await vamp.edit(
