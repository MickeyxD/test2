...
from vampBot.plugins.sql_helper.fban_sql import (
    add_channel,
    get_all_channels,
    in_channels,
    rm_channel,
...
@vampBot.on(admin_cmd(pattern="fban ?(.*)"))
@vampBot.on(sudo_cmd(pattern="fban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@vampBot.on(admin_cmd(pattern="unfban ?(.*)"))
@vampBot.on(sudo_cmd(pattern="unfban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mssg = await eor(event, "`...`")
...
@vampBot.on(admin_cmd(pattern=r"fadd ?(.*)"))
@vampBot.on(sudo_cmd(pattern=r"fadd ?(.*)", allow_sudo=True))
...
@vampBot.on(admin_cmd(pattern=r"fremove ?(.*)"))
@vampBot.on(sudo_cmd(pattern=r"fremove ?(.*)", allow_sudo=True))
...
@vampBot.on(admin_cmd(pattern="fgroups"))
@vampBot.on(sudo_cmd(pattern="fgroups", allow_sudo=True))
...
@vampBot.on(admin_cmd(pattern="fsearch ?(.*)"))
@vampBot.on(sudo_cmd(pattern="fsearch ?(.*)", allow_sudo=True))
...
@vampBot.on(admin_cmd(pattern="newfed ?(.*)", outgoing=True))
@vampBot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {vamp_input}")
...
@vampBot.on(admin_cmd(pattern="renamefed ?(.*)"))
@vampBot.on(sudo_cmd(pattern="renamefed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return 
    vamp_input = event.pattern_match.group(1)
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {vamp_input}")
              response = await response 
          except YouBlockedUserError: 
...
@vampBot.on(admin_cmd(pattern="fstat ?(.*)"))
@vampBot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    vamp = await event.edit("`Collecting fstat....`")
    thumb = vamp_logo
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        ...
        async with borg.conversation(bot) as conv:
            try:
                ...
                if "Looks like" in massive.text:
                    ...
                    massive = await conv.get_response()
                    await vampBot.send_file(
                        event.chat_id,
                        massive,
                        thumb=thumb,
                        caption=f"List of feds {user} has been banned in.\n\n**⚡ [Collected using vampBot](t.me/VAMPBOT_OFFICIAL) ⚡**",
                    )
                else:
                    await borg.send_message(event.chat_id, massive.text)
                await event.delete()
            except YouBlockedUserError:
                await vamp.edit("`Please Unblock` @MissRose_Bot")


@vampBot.on(admin_cmd(pattern="fedinfo ?(.*)"))
@vampBot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    vamp = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with borg.conversation(bot) as conv:
        try:
            await conv.send_message("/start")
            ...
            massive = await conv.get_response()
            await vamp.edit(massive.text + "\n\n**LEGENDARY_AF_vampBot**")
        except YouBlockedUserError:
            await vamp.edit("`Please Unblock` @MissRose_Bot")
