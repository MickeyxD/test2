...
@vampBot.on(admin_cmd(pattern=r"unpack", outgoing=True))
@vampBot.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@vampBot.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@vampBot.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
