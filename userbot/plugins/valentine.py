...
from vampBot import CmdHelp, bot as vampBot, ALIVE_NAME
from vampBot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

vamp = vampBot.uid

vamp = f"[{DEFAULTUSER}](tg://user?id={vamp})"

@vampBot.on(admin_cmd(pattern="hvd$"))
@vampBot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@vampBot.on(admin_cmd(pattern="vday$"))
@vampBot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
